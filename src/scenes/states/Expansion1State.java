...
public class Expansion1State extends State{

    private ArrayList<Side> sides;

    public Expansion1State(PlayingScene scene) {
        super(scene);
        System.out.println("[Expansion1State] making State");
        for(Button button : scene.controls){button.setActive(button.getText().equals("exit"));}
        for(Button button : scene.devCards){button.setActive(button.getText().equals("expansion"));}
    ...
    public void buttonClicked(String name) {
        if (name.equals("exit")) {
            ...
            scene.setState(new Expansion2State(scene));
        }
    }
